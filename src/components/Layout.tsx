...
export function Layout({ children, showBreadcrumbs = true, darkBackground = false }: LayoutProps) {
  ...
  return (
    <div className={`min-h-screen flex flex-col font-sans subtle-grid ${darkBackground ? 'bg-slate-950 text-slate-100' : 'bg-slate-50 text-slate-900'}`}>
      <Header />
      <main className="flex-grow w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <AnimatePresence mode="wait">
          <motion.div
            ...
            className={`${darkBackground ? 'bg-slate-900 border-slate-800 shadow-slate-900/50' : 'bg-[#fafaf9] border-slate-200/60 shadow-sm'} rounded-2xl border p-6 md:p-10 lg:p-12 my-6 relative overflow-hidden`}
          >
